...
class VEWorker(AsyncWorker):
    ...
    def calculate_ve(self):
        ...
        for section in self.detected_sections:
            ...
            if "outbound_start_idx" in section and "outbound_end_idx" in section:
                ...
                if (
                    ...
                    outbound_params = self.params.copy()

                    # Set comparison mode flag
                    outbound_params["comparison_mode"] = self.wind_source == "compare"

                    # For constant wind mode, remove air_speed/wind_speed columns so VE uses constant wind
                    if self.wind_source == "constant":
                        if "air_speed" in outbound_data.columns:
                            outbound_data = outbound_data.drop(columns=["air_speed"])
                        if "wind_speed" in outbound_data.columns:
                            outbound_data = outbound_data.drop(columns=["wind_speed"])
            ...
            if "inbound_start_idx" in section and "inbound_end_idx" in section:
                ...
                if (
                    ...
                    # Get current parameters
                    inbound_params = self.params.copy()

                    # Set comparison mode flag
                    inbound_params["comparison_mode"] = self.wind_source == "compare"

                    # For constant wind mode, remove air_speed/wind_speed columns so VE uses constant wind
                    if self.wind_source == "constant":
                        if "air_speed" in inbound_data.columns:
                            inbound_data = inbound_data.drop(columns=["air_speed"])
                        if "wind_speed" in inbound_data.columns:
                            inbound_data = inbound_data.drop(columns=["wind_speed"])

                    # Create VE calculator for this segment
                    # Wind/air speed data is automatically handled from DataFrame columns
                    inbound_ve_calculator = VirtualElevation(inbound_data, inbound_params)
        ...
        if has_elevation and self.all_actual_elevations and not self.params["velodrome"]:
...
class OutAndBackResult(QMainWindow):
    ...
    def get_current_params(self):
        ...
        # NOTE: For out-and-back analysis, we don't pass global wind/air speed data here
